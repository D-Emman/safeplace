...
return _fs.collection(postsCollection)
.orderBy('timestamp', descending: true)
.limit(limit)
.snapshots()
.map((snap) => snap.docs.map((d) => PostModel.fromMap(d.id, d.data())).toList());
}
...
Future<void> reportContent(String collection, String docId, String reason) async {
final ref = _fs.collection(collection).doc(docId);
await ref.update({'isReported': true, 'reportedAt': FieldValue.serverTimestamp(), 'reportReason': reason});
